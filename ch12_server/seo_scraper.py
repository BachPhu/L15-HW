...
def fetch_title():
    ...
    #raise error if invalid
        ...
        soup = BeautifulSoup(page.content, "html.parser")
        #If find title tag in html, display title. If not, display not found 
        if soup.title: 
...
#added fetch external link function
def fetch_external_links():
    url = _url.get()
    base_url = url.split("//")[-1]  # Take only the base url
    _images.set(())   # initialised as an empty tuple
    try:
        page = requests.get(url)
    except requests.RequestException as err:
        ...
        external_links = []
        #find all anchor tag with href attribute
        links = soup.find_all('a', href = True)
        
        #If find external link, add it to the external_links list. If not print so.
        if links:
            for link in links:
                href = link['href']
                #Check each link, if it is external, add to the external list 
                if href.startswith(('https', 'http')) and base_url not in href:
                    external_links.append(href)
        else:
            sb('No link found') 

        #Display the link name in the box and how many link found on status bar
        if external_links:
            _images.set(tuple(external_links))
            sb(f"External link(s): {len(external_links)}")
...
if __name__ == "__main__": # execute logic if run directly
    ...
    _fetch_img_btn = ttk.Button(
        _url_frame, text='Fetch images', command=fetch_url)
    ...
    _fetch_link_btn = ttk.Button(
        _url_frame, text='Fetch links', command=fetch_external_links)
